...
const { spinnerStart, sleep, execAsync } = require('@web-study/utlis');
const { getNpmLatestVersion } = require('@web-study/get-npm-info');
...
class InitCommand extends Command {
  ...
  async downloadTemplate() {
    ...
    const targetPath = path.resolve(userHome, '.web-study-cli', 'template');
    const storeDir = path.resolve(userHome, '.web-study-cli', 'template', 'node_modules');
    ...
    if (!await templateNpm.exists()) {
      ...
      try {
        ...
        spinner.stop(true);
        haveError? log.error('模板下载失败') : log.success('模板下载成功');
      }
      ...
      try {
        ...
        spinner.stop(true);
        haveError? log.error('模板更新失败') : log.success('模板更新成功');
      }
    }
  }
  ...
  async installNormalTemplate() {
    ...
    await sleep(800);
    spinner.stop(true);
    haveError? log.error('安装失败') : log.success('安装成功');
